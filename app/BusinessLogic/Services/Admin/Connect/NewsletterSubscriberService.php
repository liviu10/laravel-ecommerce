...
use App\Models\Admin\Connect\NewsletterSubscriber;
...
class NewsletterSubscriberService implements NewsletterSubscriberInterface
{
    ...
    {
        $unsubscribeUserPayload = [
            'column_name'  => 'email_address',
            'filter_value' => $request->get('email_address')
        ];
        $apiDisplaySingleRecord = $this->modelName->filterTableColumn($unsubscribeUserPayload);
        $fetchUserId = $apiDisplaySingleRecord->pluck('id')[0];
        ...
        if ($apiDisplaySingleRecord instanceof Collection)
        {
            ...
            {
                $apiDisplaySingleRecord = $this->modelName->deleteRecord($fetchUserId);
                return response($this->handleResponse('success', $apiDisplaySingleRecord), 200);
            }
        }
        else
    ...
    public function handleRestoreSubscription($request)
    {
        ...
        if ($apiDisplaySingleRecord instanceof Collection)
        {
            ...
            {
                $this->modelName->restoreRecord($fetchUserId);
                return response($this->handleResponse('success'), 200);
            }
        }
        else
